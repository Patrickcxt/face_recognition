...
if __name__ == '__main__':
    ...
    features = np.empty((len(images), 4096))
    for i in range(len(images)):
        print("Process %dth image: %s"%(i, images[i]))
        im = imread(images[i], mode='RGB')
        im = imresize(im, (224, 224))
        fc2 = sess.run(vgg.fc2l, feed_dict={vgg.imgs: [im]})[0]
        features[i, :] = fc2


    pickle.dump(sex_labels, open('./wiki_opencv_features/sex_labels_vgg_untuned', 'wb'))
    pickle.dump(age_labels, open('./wiki_opencv_features/age_labels_vgg_untuned', 'wb'))
    pickle.dump(features, open('./wiki_opencv_features/features_vgg_untuned', 'wb'))
